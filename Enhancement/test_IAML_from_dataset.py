# IAML: Illumination-Aware Mirror Loss for Low-light Image Enhancement
# Adapted from Retinexformer's test_from_dataset.py.
# Only three things differ from the original:
#   1. Model class and weight loading (IAMLNet instead of RetinexFormer)
#   2. Forward pass uses model_restoration.inference(input_)
#   3. --opt accepts our IAML YAML config
# Everything else — image loading, PSNR/SSIM, padding, postprocessing — is
# byte-for-byte identical to test_from_dataset.py.
...
parser = argparse.ArgumentParser(
    description='Image Enhancement using IAML')
...
parser.add_argument(
    '--opt', type=str, default='Options/IAML_LOL_v1.yml', help='Path to option YAML file.')
parser.add_argument('--weights', default='experiments/IAML_LOLv1/best_model.pth',
